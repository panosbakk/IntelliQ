...
exports.postQuestionAnswer = async (req, res) => {
  try {
    ...
    if (!questionnaireID || !session || !optionID) {
      return badRequest(
        res,
        "Invalid request. QuestionnaireID, Session and OptionID are all required parameters."
      );
    }
    
    if (!questionID) {
      return badRequest(res, "QuestionID is a required parameter.");
    }
