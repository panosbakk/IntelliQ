...
program.command("healthcheck").action(function () {
  fs.access("softeng2228.token", fs.F_OK, (err1) => {
    //File does not exist
    if (err1) {
      if (err1.code === "ENOENT") {
        console.log(
          "Error: Access denied. You should log in in order to access this page."
        );
        return;
      }
      //Another error has occured
...
program.command("resetall").action(function () {
  fs.access("softeng2228.token", fs.F_OK, (err1) => {
    //File does not exist
    if (err1) {
      if (err1.code === "ENOENT") {
...
program
  ...
  .action(function (options) {
    if (options.questionnaire_ID == undefined) {
      console.log(
        "Must define the questionnaire ID using '--questionnaire_ID' option"
      );
      return;
    }
        ...
        fs.readFile("softeng2228.token", "utf8", (err2, data) => {
          ...
          let config = {
            ...
            url:
              ...
              (options.questionnaire_ID != undefined
...
program
  ...
  .action(function (options) {
    if (options.questionnaire_ID == undefined) {
      console.log(
        "Must define questionnaire ID using '--questionnaire_ID' option"
      );
      return;
    }
